...
$uniquecode = $mysqli->escape_string($_POST['uniquecode']);
...
if ( $result->num_rows > 0 ) {
  ...
  if ($uniquecode == $code) {
        ...
        if ( $mysqli->query($sql) ){
            ...
            http://saachs.org/verify.php?email='.$email.'&hash='.$hash;

            mail( $to, $subject, $message_body );
            
            
                
               echo "<script>window.top.location='http://www.saachs.org/profile.php'</script>";
               }
      ...
      echo '<script language="javascript">';
            echo 'alert("Wrong registration unique code!! Please verify your unique code with the admin, otherwise you are not legitimate to register on this portal!!!")';
            echo '</script>';
